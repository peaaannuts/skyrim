...
## ⏭ 引継ぎ：次にやること（2026-07-18 更新）

**進行中の方針：** ①Component B本実装 → ③Component Cオーバーレイ → ②翻訳パイプライン、の順。
**①②③すべて実装・データとも完了。** ①③のE2E PoC成功（2026-07-14）に続き、
②翻訳パイプラインも抽出完了＋**翻訳100%完了**（2026-07-18、`translations.json` 4→27,840行、
DeepL 1,747件＋Azure Translator 26,093件）。
**次の一手：** 実機で `npm run build`→デプロイ→**任意のNPC**にF10で本物の日本語訳が出るか
in-game確認（＝翻訳データがフル反映された状態での最終確認。①③のPoC自体は既に成功済み）。
