...
- **DLC翻訳：** ✅ 完了（2026-07-22）。`node pipeline/translate.js --provider azure` 実行で
  DLC分7,872件を一度に完投（Skyrim分27,840件はスキップされ、想定通りDLC分のみ翻訳）。
  **`translations.json` は 35,712/35,712件＝カバレッジ100%**（Skyrim 27,840 +
  Dawnguard 3,119 + Dragonborn 4,098 + HearthFires 655）。検品済み（MOCK混入0・
  キー形式OK・空値0・長文サンプルとも訳質良好）。
...
**進捗（2026-07-21）：**
1. ✅ `native-plugin/build.bat` で再ビルド成功（`[100%]: build ok`、実機確認済み）。
2. ✅ `.psc` 再コンパイル成功（PapyrusCompiler.exe 直叩き。`houseCARL_compile_script` は
   実機に存在せず、生の `PapyrusCompiler.exe` を使用。ハマりどころ：`-import` に
   **`JpSubtitle.psc` 自身のあるフォルダ**（`native-plugin\Scripts\Source`）も含めないと
   `unable to locate script JpSubtitle` で失敗する。バニラSourceフォルダだけでは足りない。
   正しいコマンド例は下記「ビルド・デプロイ手順」に追記）。
   `Batch compile of 1 files finished. 1 succeeded, 0 failed.` を確認、
   `jp-subtitle-spike\Scripts\JpSubtitle.pex` 生成済み。

4. ✅ DLC翻訳完了（2026-07-22、上記参照）。

**残り（実機）：**
3. `.dll`/`.pdb`/`.pex` を `C:\Modding\MO2\mods\jp-subtitle-spike\...` へデプロイ
   （ゲーム起動中はDLLロックでコピー不可、閉じてから。完了状況は要確認）。
5. `npm run build` → `jp-subtitle.js` デプロイ。
6. in-game確認。ログに `[QUERY] speaker=... topicInfo=... key=Skyrim.esm|... text="..."`
   （Skyrim.esm）や `key=Dawnguard.esm|...`（Dawnguard中）が出るか確認。
   翻訳は100%カバレッジ済みなので、`miss` ではなく `hit` が出るのが期待結果。
...
**Papyrus（.psc → .pex）:**
- `houseCARL_compile_script` は実機に存在しない（2026-07-21確認、`Get-Command` でヒットせず）。
  生の `PapyrusCompiler.exe` を直接使う。**ワンクリック用に `rebuild_papyrus.bat`（リポジトリ
  直下）を用意済み**。ゲームを閉じてからダブルクリックすればコンパイル→MO2へ出力→.pex日時表示
  まで行う。コマンドを手打ちする場合：
  ```powershell
  & "C:\Program Files (x86)\Steam\steamapps\common\SkyrimVR\Papyrus Compiler\PapyrusCompiler.exe" `
    "C:\Modding\SubtitleTranslator\native-plugin\Scripts\Source\JpSubtitle.psc" `
    -output="C:\Modding\MO2\mods\jp-subtitle-spike\Scripts" `
    -import="C:\Modding\SubtitleTranslator\native-plugin\Scripts\Source;C:\Program Files (x86)\Steam\steamapps\common\SkyrimVR\Data\Source\Scripts" `
    -flags="TESV_Papyrus_Flags.flg"
  ```
  **2つのハマりどころ（両方 2026-07-22 実機で踏んだ）：**
  1. `-import` に `JpSubtitle.psc` 自身のあるフォルダ（`native-plugin\Scripts\Source`）を
     含めないと `unable to locate script JpSubtitle` で失敗（コンパイラは `-import` 内で
     オブジェクト名を検索して解決する仕様）。
  2. フラグ定義ファイル `TESV_Papyrus_Flags.flg` は **SkyrimVRの `Data\Source\Scripts\`**
     （VRレイアウトは `Source\Scripts` の順）にある。SSE側の `Data\Scripts\Source` には
     無いので、`-import` にVRのソースフォルダを入れないと `Unknown user flag Hidden` で失敗。
- native関数を追加/変更したら **C++登録・.psc宣言・.pex再コンパイルの3つを必ず揃える**。
- **落とし穴：`.pex` の更新確認を怠ると事故る。** 2026-07-22、`.psc` を
  `GetCurrentDialogueKey` に変えたのに `.pex` が古い（07-14の88バイト）ままで、in-gameで
  `callNative failed: Native function not found 'JpSubtitle.GetCurrentDialogueKey'` が
  多発。原因は上記フラグエラーでコンパイルが実は失敗し続けていたこと。再コンパイル後は
  必ず `.pex` の `LastWriteTime` が「今」になったか確認する。
