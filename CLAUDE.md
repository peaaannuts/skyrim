...
- **DLC翻訳：** ❌ 未実施。実機で `node pipeline/translate.js --provider azure` を
  ...
  される設計）。
- **C++（Component B）：** ✅ 実装済み・**未ビルド・未実機検証**。`native-plugin/` を
  ようやくGitHubへpush（下記「リポジトリ状態の注意」参照）。`GetCurrentDialogueFormID`
  （Int返却・Skyrim.esm専用）を `GetCurrentDialogueKey`（String返却、
  `"<Plugin>|<FORMID6>"` 形式、`TESForm::GetFile(0)->GetFilename()` でプラグイン名を
  解決）に置き換えた。`.psc` も追随済み。**CommonLibSSE-NGのヘッダがLinux側に無く
  コンパイル未検証**（`RE::TESForm::LookupByID` / `TESForm::GetFile` /
  `RE::BSFixedString` の使用はCommonLibSSE-NGの標準的なAPIだが、実機ビルドで
  コンパイルエラーが出た場合はエラーメッセージをそのまま連携してほしい）。
