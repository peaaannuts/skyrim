...
- **Component A：** **抽出完了・翻訳は初月分完了（2026-07-16）**。
  ...
  - 翻訳：`pipeline/translate.js`（DeepL、resumable、月次予算キャップ、`hard`戦略＝長文優先）で
    **今月分50万文字を消費し `translations.json` が 4行 → 1,747行**に。検品済み
    （MOCK混入0・キー形式OK・シード4行保護・訳質良好）。
  - 残り：約24,861ユニーク文字列（≈180万文字≈DeepL Free 4ヶ月分）。**8月以降に
    `node pipeline/translate.js` 再実行で月次リセットから自動継続**。
...
## ⏭ 引継ぎ：次にやること（2026-07-14 更新）

**進行中の方針：** ①Component B本実装 → ③Component Cオーバーレイ → ②翻訳パイプライン、の順。
**①②③すべて実装完了・in-game実証済み。** ①③のE2E PoC成功（2026-07-14）に続き、
②翻訳パイプラインも抽出完了＋初月翻訳完了（2026-07-16、`translations.json` 4→1,747行）。
**次の一手：** 実機で `npm run build`→デプロイ→長台詞NPCにF10で本物のDeepL訳が出るか
in-game確認（＝3コンポーネント一気通貫の最終確認）。以降は毎月の翻訳継続（下記2節）。

**リポジトリ状態の注意（Linux側・2026-07-14）：**
...
- **`native-plugin/`（C++・Component B）はまだ GitHub に未 push。** in-game検証は実機の
  デプロイ済み DLL/pex で成功しているが、Linux側でC++をレビュー/改修するには
  実機から `native-plugin/` を push する必要がある（`build/` はgitignore済みで除外される）。
...
### 2. ✅ ほぼ完了：② 翻訳パイプライン（Component A）

**抽出は完了、翻訳は初月分完了（2026-07-16）。** パイプライン3スクリプトを実装・実機実証済み：
...
- `pipeline/translate.js`：DeepL バッチ翻訳。resumable（`data/translate_progress.json`）、
  月次予算キャップ（`monthly_char_budget`）、月替わり自動リセット、シード保護、
  `DEEPL_MOCK=1` ドライラン、`--strategy hard`（既定＝長文=読解に助けが要る行を優先）。

**残タスク（来月以降）：** 今月分50万文字消費済み。残り約24,861ユニーク文字列（≈180万文字）を
**8月以降に `node pipeline/translate.js` 再実行で継続**（月次リセットで自動再開、`hard`戦略で
長文から埋まる）。完了ごとに `translations.json` をコミット→ `npm run build` → デプロイ。
- **予算判断（確定）：** 低価値な短バーク除外は文字数を1割も減らせない（課金は文字数、
  短行は元々安い）ため無意味。方針は「重要度優先（hard）で Free枠を毎月消化」で確定。
