...
- `houseCARL_compile_script` は実機に存在しない（2026-07-21確認、`Get-Command` でヒットせず）。
  ...
  & "C:\Program Files (x86)\Steam\steamapps\common\SkyrimVR\Papyrus Compiler\PapyrusCompiler.exe" `
    ...
    -import="C:\Modding\SubtitleTranslator\native-plugin\Scripts\Source;C:\Program Files (x86)\Steam\steamapps\common\Skyrim Special Edition\Data\Scripts\Source"
  ```
  **`-import` に `JpSubtitle.psc` 自身のあるフォルダ（`native-plugin\Scripts\Source`）を
  含めないと `unable to locate script JpSubtitle` で失敗する**（コンパイラは渡されたパスを
  信用せず、`-import` 内でオブジェクト名を検索して解決する仕様のため。バニラSourceフォルダ
  だけでは不十分）。実機で成功確認済み（`1 succeeded, 0 failed`）。
